...
int main(int argc, char **argv)
{
	...
	if(argc < 2)
	{
		fprintf(stderr,"\nMATH_ERROR: You must give the IP in order to connect.\n");
		exit(EXIT_FAILURE);
	}

	if(argc < 3)
	{
		fprintf(stderr,"\nMATH_ERROR: You must give a number of students.\n");
		exit(EXIT_FAILURE);
	}

	if(argc < 4)
	{
		fprintf(stderr,"\nMATH_ERROR: Zero messages have been entered.\n");
		exit(EXIT_FAILURE);
	}
	...
	{
		perror("\nsocket() error.\n");
		fprintf(stderr,"MATH_ERROR: errno = %d.\n", errno);
		exit(EXIT_FAILURE);
	}
	else
	{
		fprintf(stdout,"\nMATH_Client: socket is ready ..\n");
		sleep(2);
	}
	...
	{
		fprintf(stdout,"\nMATH_Client: Connextion Established!\n");
		sleep(2);
	}
	...
	for(j=3;j<argc;j++)
	{
		...
		if(strcmp("GETM",buffer) == 0)
		{
			...
			if(Rnumbytes > 0 )
			{
				...
				fprintf(stdout,"\nMATH: recieved the key :%d\n",mem_key);
				if(attach_shared_memory() == 0)
				{
					close(sock);
					fprintf(stderr,"MATH_ERROR: cant attach shared memory\n");
					exit(EXIT_FAILURE);
				}
				else
				{
					fprintf(stdout,"\nMATH: shared memory attached!\n");
					students = atoi(argv[2]);
					fprintf(stdout,"\nMATH: The total students are : %d\n",students);					
				}
			}
			else if(numbytes == 0)
			{
				fprintf(stdout,"\nMATH: recieved 0 bytes from Server.\n");
			}
			else
			{
				close(sock);
